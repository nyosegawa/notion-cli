...
1. `notion --help` → コマンド一覧 + Quick start（基本ワークフロー・DB のコツ・`--json` 推奨）
2. いきなり実行する（Agent は `--help` を深掘りせず試す）
3. エラーが出たら Hint に従って修正する
4. 複雑な操作は `notion <command> --help` で例とヒントを確認する
...
- `notion --help` → 全コマンド一覧 + Quick start (ワークフロー・基本例・`--json` 推奨・per-command --help への案内)
- `notion page --help` → サブコマンド一覧
- `notion db create --help` → フラグ・例・前提条件・次のステップ (`addHelpText` で付加)
...
Error: notion-create-pages failed
  ...
  Hint: If adding to a database, use --data with "parent":{"data_source_id":"<ds-id>",...}. Run "notion fetch <db-id>" to get the data_source_id
```
...
MCP ツール名や MCP の内部構造は Agent に見せない（`notion api` で必要な場合のみ）。
...
CLI にまだ対応していないツールや複雑な引数構造には `notion api` で対応。
...
Agent は `notion --help` でコマンド一覧を確認し、CLI コマンドが不十分なら `notion api` にフォールバックできる。
