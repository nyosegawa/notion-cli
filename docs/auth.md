...
MCPConnection.connect()
  ...
  │   │   └── callback-port.json にコールバックポートも保存
...
├── client.json           # OAuth client registration (client_id, client_secret)
├── tokens.json           # access_token, refresh_token, expires_at
├── auth-state.json       # codeVerifier (一時、logout 時にクリア)
└── callback-port.json    # コールバックサーバーのポート (redirect_uri 再利用用)
```
...
| `clientInformation()` / `saveClientInformation()` | TokenStore 経由で client.json を読み書き + callback-port.json にポート保存 |
...
**対策**: `saveClientInformation()` 時にポートを `callback-port.json` に保存し、次回 `connect()` 時に同じポートで `CallbackServer.start()` を呼ぶ。

- ポート競合時は `listen(0)` にフォールバックし、`client.json` を破棄して再登録
- 旧バージョンからのアップグレード時（`callback-port.json` なし + `client.json` あり）も同様に破棄して再登録
