...
| メソッド | 実装 |
|---|---|
| `redirectUrl` | `http://localhost:{auto-port}/callback` |
| `clientMetadata` | `{ client_name: "notion-cli", grant_types: [...], token_endpoint_auth_method: "none" }` |
...
| `redirectToAuthorization(url)` | CallbackServer 起動 + `open(url)` |
